    ...
    <div
      ...
      <div class="flex justify-center">
        <nav class="flex overflow-x-auto items-center p-1 space-x-1 text-sm text-gray-600 bg-gray-100 rounded-xl">
          <button role="tab" type="button" :class="['flex whitespace-nowrap items-center h-8 px-5 font-medium rounded-lg outline-none transition-all duration-300',
            selectedSHA === 'SHA-1'
              ? 'text-[#FF8000] bg-white shadow focus:ring-2 focus:ring-[#FF8000] focus:ring-inset'
              : 'hover:text-gray-800 focus:text-[#FF8000]']" @click="selectedSHA = 'SHA-1'">
            SHA-1
          </button>
          ...
          <button role="tab" type="button" :class="['flex whitespace-nowrap items-center h-8 px-5 font-medium rounded-lg outline-none transition-all duration-300',
            selectedSHA === 'SHA-256'
              ? 'text-[#FF8000] bg-white shadow focus:ring-2 focus:ring-[#FF8000] focus:ring-inset'
              : 'hover:text-gray-800 focus:text-[#FF8000]']" @click="selectedSHA = 'SHA-256'">
            SHA-256
          </button>
        </nav>
      </div>
      ...
      <div>
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <input v-model="secretKey" type="password" id="secretkey"
              class="py-2 px-3 block w-full border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-[#FF8000] focus:border-[#FF8000]"
      ...
      <div>
        ...
        <div class="space-y-4">
          <div>
            ...
            <input v-model="referencia" type="text" id="referencia"
              class="py-2 px-3 block w-full border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-[#FF8000] focus:border-[#FF8000]"
          ...
          <div>
            ...
            <input v-model="sesion" type="text" id="sesion"
              class="py-2 px-3 block w-full border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-[#FF8000] focus:border-[#FF8000]"
      ...
      <div class="flex flex-wrap gap-3">
        ...
        <button @click="mostrarConsulta = !mostrarConsulta"
      ...
      <div v-if="mostrarConsulta" class="mt-4 border-t border-gray-200 pt-4 space-y-4">
        ...
        <input v-model="consultaEndpoint" id="consultaEndpoint" type="url"
        ...
        <input v-model="consultaRequestId" id="consultaRequest" type="text"
          class="py-2 px-3 block w-full border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          placeholder="Ej: 654321" />

        <button @click="consultarSesion"
...
async function consultarSesion() {
  ...
  try {
    ...
    try {
      ...
      respuestaServidor.value = JSON.stringify(parsed, null, 2)
    } catch {
      respuestaServidor.value = text?.trim() || '(Sin contenido JSON válido)'
    }
  } catch (error) {
...
// === ENVIAR DATOS (actualizado para manejar JSON o texto plano) ===
// === ENVIAR DATOS (mejorado para manejar cualquier tipo de respuesta) ===
async function enviarDatos() {
  ...
  // Firma dinámica según SHA seleccionado
  let firmaHash =
  ...
  let firma = selectedSHA.value === 'SHA-256' ? `sha256:${firmaHash}` : firmaHash;

  // 🔹 Aquí requestId se envía como número entero (sin comillas)
  const data = {
    status: {
    ...
    requestId: Number(sesion.value) || 0, // <-- se envía como número
  ...
  try {
    ...
    let info = `📡 Código HTTP: ${response.status} ${response.statusText}\n\n`;

    // 🔹 Intentar leer la respuesta como texto o JSON
    const text = await response.text();
    try {
      const parsed = JSON.parse(text);
      respuestaServidor.value = info + JSON.stringify(parsed, null, 2);
    } catch {
      respuestaServidor.value = info + (text?.trim() || '(Sin contenido o texto plano)');
    }
  } catch (error) {
